...
def question_five(conn):
    with conn:
        cursor = conn.cursor()
        query = cursor.execute("""SELECT c.CustomerID, c.CompanyName, c.ContactName, c.Phone, c.Fax 
        FROM Customers c 
        WHERE c.Fax IS NULL """)
        while True:
            row = query.fetchone()
...
def question_seven(conn):
    with conn:
        cursor = conn.cursor()
        query = cursor.execute("""SELECT TOP 5 c.CustomerID, c.ContactName, SUM(od.Quantity) 
        FROM Customers c 
...
def question_eight(conn):
    with conn:
        cursor = conn.cursor()
        query = cursor.execute("""SELECT TOP 5 c.CustomerID, c.ContactName, c.CompanyName, c.Phone, c.Address, SUM(od.TQuan), COUNT(od.OrderID) 
        FROM Customers c 
        INNER JOIN Orders o ON c.CustomerID = o.CustomerID 
